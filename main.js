$(document).ready(function(){
	...
	// shop() is a function fetching shop record from database whenever page is load
	/*
	function shop(){
		$.ajax({
			url: "action.php",
	...
	*/
	...
	nav_marque(); //est une fonction permet de recuperer les marques et les afficher sous forme d'une ligne en haut de footer
	function nav_marque(){
		$.ajax({
			url	:	"action.php",
			method:	"POST",
			data	:	{nav_marque_list:1},
			success	:	function(data){
				$("#nav_marque").html(data);
			}
		})
	}
	...
	function caroussel(){
		$.ajax({
			url	:	"action.php",
			method:	"POST",
			data	:	{get_caroussel:1},
			success	:	function(data){
				$("#get_caroussel").html(data);
			}
		})
	}
	...
	function product_one(){
		...
		$.ajax({
			url	:	"action.php",
			method:	"POST",
			data	:	{getProduct_one:1, url:window.location.search}/*,data: "getProduct_one="+1+"&url="+window.location.href*/,
			success	:	function(data){
				$("#get_product_one").html(data);
			}
		})
	}
	function product_search(){
		...
		$.ajax({
			url	:	"action.php",
			method:	"POST",
			data	:	{get_product_search:1, url:url}/*,data: "getProduct_one="+1+"&url="+window.location.href*/,
			success	:	function(data){
				$("#get_product_search").html(data);
			}
		})
	}
	...
	*/
	...
	$("body").delegate(".nav_marque","click",function(event){//DMK
		event.preventDefault();
		$("#get_product").html("<h3>Chargement...</h3>");
		var mid = $(this).attr('mid');
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{nav_marque:1,marque_id:mid},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
	...
	$("body").delegate(".selectSousCat","click",function(event){
		event.preventDefault();
		$("#get_product").html("<h3>Chargement...</h3>");
		var scid = $(this).attr('scid');
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{selectSousCat:1,sous_cat_id:scid},
			success	:	function(data){
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
